...
export const WindowLayerContext = React.createContext(
  {} as WindowLayerContextState
);

type WindowLayerProviderProps = {
  children: React.ReactNode;
};
const WindowLayerProvider = observer(
  ({ children }: WindowLayerProviderProps) => {
    ...
    const createWindow = (window: AppWindow) => {
      ...
      switch (window.type) {
        case WindowType.windowR:
          content = <WindowR id={window.id} {...window.props} />;
          break;
        case WindowType.windowG:
          content = <WindowG id={window.id} {...window.props} />;
          break;
        case WindowType.windowB:
          content = <WindowB id={window.id} {...window.props} />;
          break;
      }
      let frame = <></>;
      switch (window.frame) {
        ...
        case WindowFrameType.windowFrameR:
          frame = (
            <WindowFrameR onClose={() => closeWindow(window.id)}>
              {content}
            </WindowFrameR>
          );
          break;
        case WindowFrameType.windowFrameG:
          frame = (
            <WindowFrameG onClose={() => closeWindow(window.id)}>
              {content}
            </WindowFrameG>
          );
          break;
      ...
      return (
        <Draggable
          initPos={window.initPos}
          getNextHighestZIndex={getNextHighestZIndex}
        >
          {frame}
        </Draggable>
      );
